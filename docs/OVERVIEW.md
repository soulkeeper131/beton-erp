...
> ⚠️ **Н-18 / фискални устройства не са в обхвата** — фирмата не издава касови бележки и не приема кеш от физически лица; работи по фактури (ЗДДС) + банков превод. Фокусът е да работи ERP-то, не фискализация.

### 🔴 Фаза 1 — Завършване на ядрото (висок приоритет)

1. **Активиране на „мъртвите" таблици** (вече в schema, липсва UI/API):
...
2. **Разширяване на ролите** — `manager`, `brigadir` (README ги споменава, реално липсват). Специфични права: бригадир — само календар + снимки на обекта.

### 🟡 Фаза 2 — Отчетност и операции (среден приоритет)

3. **Разширени справки/отчети** — печалба по обект/клиент, разход по машина, справка за материали, сравнение „офертирано vs. актувано" в по-дълбок детайл.
4. **Складова книга** — пълен приход/разход с history (ползва `material_deliveries`).
5. **Автоматични нотификации** — изтичащи винетки/ГО/прегледи (данните вече са в `machines`), ниски наличности, неплатени фактури.
6. **Recurring оферти/фактури** — месечни доставки, договорни цени.

### 🟢 Фаза 3 — Качество и мащаб (нисък приоритет)

7. **Автоматични тестове** — в момента няма нито един тест (няма test framework). Минимум: API smoke tests + PDF генерация.
8. **Backup автоматизация** — offsite (S3/Drive) + криптиране, а не само локална ротация.
9. **Изчистване на schema drift** — уеднаквяване на `schema.ts` ↔ `index.ts` миграциите (преминаване към единна миграционна стратегия).
10. **PWA push notifications** за наряди и срокове (manifest + sw вече има).
11. **Многоезичност** — ниско приоритетно (вътрешен инструмент, български е достатъчен).
